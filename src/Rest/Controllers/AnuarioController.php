...
class AnuarioController
{
    private AnuarioService $service;
    public function __construct(AnuarioService $service)
    {
        $this->service = $service;
    }
    public function listar()
    {
        $listaArchivos = $this->service->listar();
        return [
            'data' => $listaArchivos,
            'total' => count($listaArchivos)
        ];
    }
    ...
    public function descargarArchivo(WP_REST_Request $request)
    {
        ...
        header('Content-length: '.filesize($archivo->path));
